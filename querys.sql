...
ON equipment.bild_id = equipmentbild.bild_id WHERE equipment.geloescht=false AND equipment.aktiv=true AND (buchung.reserviert_fuer = DATE(NOW()) OR buchung.reserviert_fuer IS NULL) ORDER BY set_.name DESC